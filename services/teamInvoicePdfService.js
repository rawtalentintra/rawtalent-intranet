...
function buildTeamInvoicePdf(invoice, recipient) {
  return new Promise((resolve, reject) => {
    ...
    function layoutBankBlock(top, measure) {
      ...
      bankY2 += field(RIGHT_X, bankY2, 'Swift Code', recipient.bank_swift_code || '-', RIGHT_VALUE_W, RIGHT_LABEL_W, measure) + 3;
    ...
    doc.y += 8;
    doc.rect(LEFT_X, doc.y, CONTENT_WIDTH, ROW_H + 8).fill('#f3f4f6');
    doc.font('Helvetica-Bold').fontSize(10).fillColor(NAVY)
      .text('Total Pay in AUD', LEFT_X + 10, doc.y + 5, { width: 340, lineBreak: false });
    doc.text(fmtMoney(invoice.totalAud), colAmountX, doc.y + 5, { width: 75, align: 'right', lineBreak: false });
    doc.y += ROW_H + 8;
