...
// ── admin, and qa_view/workforce_partner all get this: writing/correcting ──
// ── FAQ answers, with no access to the raw Slack/Fathom scan or review ──
// ── queue (still super_admin-only below). ──
router.get('/manage/all', requireRole('super_admin', 'admin', 'qa_view', 'workforce_partner'), async (req, res) => {
  try {
...
router.post('/', requireRole('super_admin', 'admin', 'qa_view', 'workforce_partner'), async (req, res) => {
...
router.put('/:id', requireRole('super_admin', 'admin', 'qa_view', 'workforce_partner'), async (req, res) => {
  const { question, answer } = req.body;
