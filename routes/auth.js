...
router.get('/me', async (req, res) => {
  ...
  const info = { email: req.user.email, name: req.user.name, role: req.user.role, canBuildTraining: !!req.user.can_build_training, canCreateOutreachLists: !!req.user.can_create_outreach_lists, canUseWfpPwa: !!req.user.can_use_wfp_pwa, canCalibrateCalls: req.user.role === 'super_admin' || !!req.user.can_calibrate_calls, wfpLabel: req.user.wfp_label || null, canViewAllTerritories: !!req.user.can_view_all_wfp_territories, additionalTerritories: Array.isArray(req.user.additional_wfp_territories) ? req.user.additional_wfp_territories : [], isPayrollAdmin: isFinalApprover(req.user.email) };
