  ...
  try {
    ...
    if (additionalWfpTerritories !== undefined) {
      ...
      if (JSON.stringify([...newList].sort()) !== JSON.stringify([...oldList].sort())) {
        await db.execute({ sql: 'UPDATE users SET additional_wfp_territories = ? WHERE id = ?', args: [JSON.stringify(newList), req.params.id] });
        changes.push(`Additional territories: "${oldList.join(', ') || '—'}" → "${newList.join(', ') || '—'}"`);
      }
    }
